...
**核心原则：长期任务开始前，先花 30 秒规划。**

### 规划三步
1. **拆解** — 把大任务拆成独立子任务
2. **识别依赖** — 哪些有先后关系，哪些互相独立
3. **并行执行** — 无依赖的子任务同时启动（用 sub-agent 或并发工具调用）

### 并行化原则
- 多个搜索/调研方向 → 同时发起，不要串行等
- 多个文件读写互不依赖 → 一次性并发
- 需要多个 API 调用且无依赖 → 同一个 function_calls 块里一起发
- Sub-agent 适合：耗时长、可独立完成的子任务
- 主线程负责：协调、汇总、与用户沟通
